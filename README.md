...
`GEMINI_KEY` — searches the web and reads the reviews
`RESEND_KEY` — sends the report

There is no second search service to pay for: the search is done through Gemini's own
Google Search grounding, so every finding comes back with the page it came from.
...
Every rule is a check in `selftest.js` — it reports `24 passed, 0 failed` and runs before
